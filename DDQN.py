...
class DDQN(Algorithm):
	def __init__(self,env,actions_size=-1):
		...
		self.t_network = Actor(self.env,actions_size)
		self.actions_size=0
		if actions_size==-1:
			self.actions_size=self.env.action_space.n
		else:
			self.actions_size=actions_size
		self.replace_t_network = 1000
		self.optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)
		self.q_network.compile(loss='mse',optimizer=self.optimizer)
		self.t_network.compile(loss='mse',optimizer=self.optimizer)
		self.nb_train=0
		self.batch_size=128
	...
	def learn(self):
		if self.last_memory < self.batch_size:
			return
		if self.nb_train % self.replace_t_network == 0:
			self.update_t_network()
		st, act, rew, n_st, don = self.get_memory()
		target_q_values = self.t_network(n_st)
